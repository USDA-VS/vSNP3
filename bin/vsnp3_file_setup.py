...
class Banner:
    ...
    def __init__(self, title, hexcode="56, 68, 117"):
        ...
        fill_color = 'rgb({})'.format(hexcode)
        svgimg.add(svgimg.rect([0, 0], [width, height], rx=None, ry=None, fill=fill_color, stroke="black"))
        #text to banner
        ...
        banner_filename = '{}-banner.png'.format(title)
        svg2png(bytestring=content, write_to=banner_filename)
        os.remove('banner.svg')
        self.banner = os.path.join(os.getcwd(), banner_filename)
...
class Latex_Report:
    ...
    def __init__(self, sample_name, config_file=None, report_description=None):
        ...
        tex_file = '{}_{}_report.tex'.format(sample_name, date_stamp)
        ...
        if logo_path and os.path.exists(logo_path):
            logo_include = r'\lhead{\begin{picture}(0,0) \put(0,0){\includegraphics[width=40mm]{' + logo_path + r'}} \end{picture}}'
            print(logo_include, file=tex)
        
        current_date = datetime.now().strftime('%B %d, %Y')
        date_header = r'\rhead{\begin{date} \textbf{\large ' + current_date + r'} \end{date}}'
        print(date_header, file=tex)
        print(r'\begin{adjustbox}{width=1\textwidth} \\', file=tex)
        if report_description:
            description_line = r'\centerline{\textbf{\large{' + report_description + r'}}}'
            print(description_line, file=tex)
        print(r'\end{adjustbox} \\', file=tex)
        print(r'\vspace{0.5 mm} \\', file=tex)
        sample_line = r'\textbf{\large {\fontfamily{\sfdefault}\selectfont Sample: ' + sample_name + r'}}'
        print(sample_line, file=tex)
        print(r'\vspace{0.5 mm}', file=tex)
    ...
    def latex_ending(self):
        ...
        self.tex.close()
        pdflatex_cmd1 = 'pdflatex -interaction=nonstopmode {} > /dev/null 2>&1'.format(self.tex_file)
        pdflatex_cmd2 = 'pdflatex -interaction=nonstopmode {} > /dev/null 2>&1'.format(self.tex_file)
        os.system(pdflatex_cmd1)
        os.system(pdflatex_cmd2)
