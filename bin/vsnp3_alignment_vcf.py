...
class Alignment(Setup):
    ...
    def __init__(self, FASTQ_R1=None, FASTQ_R2=None, reference=None, nanopore=False, gbk=None, skip_assembly=None, debug=False):
        '''
        Start at class call
        '''
    ...
    def run(self,):
        ...
        prenodup_bamfile = f'{sample_name}_prenodup.bam'
        ...
        if self.nanopore: 
            ...
            os.system(f'minimap2 -a -x map-ont -R "@RG\\tID:{sample_name}\\tSM:{sample_name}\\tPL:ILLUMINA\\tPI:250" -t 8 {reference} {self.FASTQ_R1} -o {samfile}')
        elif self.paired:
            os.system(f'minimap2 -t 8 -a -x sr {reference} {self.FASTQ_R1} {self.FASTQ_R2} -o {samfile}')
            self.aligner = "Minimap2"
        else:
            os.system(f'minimap2 -t 8 -a -x sr {reference} {self.FASTQ_R1} -o {samfile}')
            self.aligner = "Minimap2"

        # http://www.htslib.org/workflow/fastq.html
        os.system(f'samtools fixmate -O bam,level=1 -m {samfile} {fixmate_bamfile}')
        os.system(f'samtools sort -l 1 -@8 -o {pos_srt_bamfile} {fixmate_bamfile}')
        os.system(f'samtools markdup -f markduplicate_stats.txt -r -O bam,level=1 {pos_srt_bamfile} {nodup_bamfile}')
        ...
        if self.nanopore:
            ...
            os.system(f'vcffilter -f "QUAL > 20" {unfiltered_hapall} > temp1.vcf')
            ...
            chrom_ranges.close()
            os.system(f'freebayes-parallel chrom_ranges.txt 8 -E -1 -e 1 -u --strict-vcf -f {reference} {nodup_bamfile} > {unfiltered_hapall} 2>/dev/null')
        ...
        if self.skip_assembly or self.nanopore:
            ...
            try:
                ...
                try:
                    ...
                    pass
                if not self.debug and not self.skip_assembly:
                    shutil.rmtree('spades_assembly')
...
if __name__ == "__main__": # execute if directly access by the interpreter
    ...
    alignment = Alignment(FASTQ_R1=args.FASTQ_R1, FASTQ_R2=args.FASTQ_R2, reference=concatenated_FASTA, nanopore=args.nanopore, gbk=args.gbk, skip_assembly=args.skip_assembly, debug=args.debug)
    alignment.run()
