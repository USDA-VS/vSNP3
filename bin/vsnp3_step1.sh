...
py-fsspec-2021.7.0-gcc-9.2.0-rq6fs5y #step 2
...
currentdir=`pwd`
for f in ./*/; do 
    echo "starting: $f: using $@"
    cd ./$f
    file_count=$(ls *gz | wc -l)
    if [[ $file_count == 1 ]]; then
        echo "Using reference:  $@ on single read"
        ${HOME}/git/gitlab/vsnp3/bin/vsnp3_step1.py -r1 *.fastq.gz $@ &
    else        
        echo "Using reference:  $@ on paired reads"
        ${HOME}/git/gitlab/vsnp3/bin/vsnp3_step1.py -r1 *_R1*.fastq.gz -r2 *_R2*.fastq.gz $@ &
    fi
    cd $currentdir
done && no_error=true || no_error=false
wait
